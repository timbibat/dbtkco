...
fetch('/api/accessories')
  ...
  .then(data => {
    Products = data.map((p, index) => ({
      ...p,
      id: `accessories_db-${p.id || index + 1}`,
      source: 'accessories_db'
    }));
...
function setupAddToCartButtons(gridElement) {
  ...
  addToCartButtons.forEach(btn => {
    btn.addEventListener('click', function (e) {
      ...
      // Removed size-related elements
      const quantitySelect = card.querySelector('.quantity-select');

      // Removed size variable and validation
      ...
      if (typeof cart !== 'undefined' && cart.addItem) {
        // Removed size argument from addItem
      ...
      // Removed sizeSelect.value reset
